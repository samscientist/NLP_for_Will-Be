...
class SummarizeReport extends SummarizeService {
  ...
  final _refineDailyReportPrompt = PromptTemplate.fromTemplate(
    // 'Write a concise summary in Korean of this content: {context}',
    '''
    너는 특수 교육 현장의 교사가 학생의 문제행동의 정확한 원인을 찾기 위해 일주일 간의 기록을 정리하는 역할을 맡을 거야.
    ...
    - 'stamps' 안에 기록된 발생 시각의 개수만으로 행동의 횟수를 추출.
    - 특이 사항의 주요 내용 반드시 포함. 
    - 형식: "<상황>에서 <행동>을 하고, 이에 따라 <조치>를 함. / (<횟수>회). 이에 대한 특이 사항으로는 <특이사항>."
  ...
  final _summarizeWeekPrompt = PromptTemplate.fromTemplate(
    ...
    - {record} 내 구분은 '요일'의 형식으로 묶어서 정리, 순서대로 나열
    - 원본 내용과 다른 내용은 포함하지 않음
    - 한글 맞춤법 검사
    - 예시 출력 (내용의 생략은 '...'으로 처리)
      - ```
        월 - 오전 수업 시간에 의자에 앉아야 하는 상황에서 의자에 앉지 않고 누워서 소리지름(3회). 이에 따라 환경 분리 조치를 취함. 하지만 행동은 나아지지 않고 오히려 심해짐.
        화 - ... / (...회)
        수 - ... / (...회)
        목 - ... / (...회)
        금 - ... / (...회)
